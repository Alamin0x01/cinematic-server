...
async function run() {
  try {
    ...
    const runningCinemasCollections = client
      .db("cinematicDb")
      .collection("runningcinemas");
    const upcommingCinemasCollections = client
      .db("cinematicDb")
      .collection("upcommingcinemas");
    const selectedmoviesCollection = client
      .db("cinematicDb")
    ...
    app.put("/selectedmovies/:id", async (req, res) => {
      ...
      const result = await selectedmoviesCollection.updateOne(
        query,
        newUpdatedTicket,
        options
      );
